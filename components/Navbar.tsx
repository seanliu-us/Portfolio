...
const Navbar: React.FC = () => {
  ...
  const navLinks = [
    ...
    { name: "~/profile", href: "#achievements" },
    { name: "~/contact", href: "#contact" },
  ];

  return (
    <nav
      className={`fixed w-full z-50 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${
        isScrolled
          ? "glass h-16"
          : "bg-transparent h-20 border-b border-transparent"
      } animate-[slideDown_0.6s_ease-out_forwards]`}
    >
      <div className="max-w-6xl mx-auto px-6 h-full flex items-center justify-between">
        ...
        <div className="flex-shrink-0 flex items-center gap-2 group cursor-pointer">
          ...
          <span className="font-mono font-semibold text-sm tracking-tight text-zinc-200 group-hover:text-white transition-colors duration-300">
            sean_liu
            <span className="animate-cursor-blink text-emerald-500">_</span>
          </span>
        </div>
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden absolute w-full left-0 top-full bg-black/95 backdrop-blur-xl border-b border-zinc-800 overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${
